...
sudo openssl req -newkey rsa:2048 -nodes -keyout /etc/pki/tls/private/paje.key -x509 -days 365 -out /etc/pki/tls/certs/paje.crt \
-subj "/C=UK/ST=London/L=London/O=paje/OU=paje-project/CN=$(curl -s http://169.254.169.254/latest/meta-data/local-hostname)"


sudo sed -i 's/localhost.crt/paje.crt/g'  /etc/httpd/conf.d/ssl.conf

sudo sed -i 's/localhost.key/paje.key/g'  /etc/httpd/conf.d/ssl.conf
